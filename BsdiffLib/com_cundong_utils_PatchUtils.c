...
#include "com_snailgame_fastdev_util_PatchUtils.h"

#define log(...) __android_log_print(ANDROID_LOG_INFO, "ApkPatchLibrary", ##__VA_ARGS__)
...
int applypatch(int argc, char * argv[]) {
	...
	if (argc != 4) {
		log("usage: %s oldfile newfile patchfile\n", argv[0]);
		return -1;
	}
	...
	if (fseeko(cpf, 32, SEEK_SET)) {
		log("fseeko(%s, %lld)", argv[3], (long long) 32);
		return -1;
	}
	if ((cpfbz2 = BZ2_bzReadOpen(&cbz2err, cpf, 0, 0, NULL, 0)) == NULL) {
		log("BZ2_bzReadOpen, bz2err = %d", cbz2err);
		return -1;
	}
	...
	if (fseeko(dpf, 32 + bzctrllen, SEEK_SET)) {
		log("fseeko(%s, %lld)", argv[3], (long long) (32 + bzctrllen));
		return -1;
	}
	if ((dpfbz2 = BZ2_bzReadOpen(&dbz2err, dpf, 0, 0, NULL, 0)) == NULL) {
		log("BZ2_bzReadOpen, bz2err = %d", dbz2err);
		return -1;
	}
	...
	if (fseeko(epf, 32 + bzctrllen + bzdatalen, SEEK_SET)) {
		log("fseeko(%s, %lld)", argv[3],
				(long long) (32 + bzctrllen + bzdatalen));
		return -1;
	}
	if ((epfbz2 = BZ2_bzReadOpen(&ebz2err, epf, 0, 0, NULL, 0)) == NULL) {
		log("BZ2_bzReadOpen, bz2err = %d", ebz2err);
		return -1;
	}
...
/*
 * Class:     com_snailgame_fastdev_util_PatchUtils
 * Method:    patch
 * Signature: (Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)I
 */
JNIEXPORT jint JNICALL Java_com_snailgame_fastdev_util_PatchUtils_patch(JNIEnv *env,

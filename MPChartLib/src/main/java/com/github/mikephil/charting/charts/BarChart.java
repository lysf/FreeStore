...
public class BarChart extends BarLineChartBase<BarData> {
            ...
            for (int i = 0; i < mCurrentData.getDataSetCount(); i++) {
                ...
                if (!mDrawValuesForWholeStack) {
                    ...
                    for (int j = 0; j < valuePoints.length * mPhaseX; j += 2) {
                        ...
                        String data = (String)entries.get(j / 2).getData();//add by licong for different unit
                        drawValue(mValueFormat.format(val), valuePoints[j],
                                valuePoints[j + 1] + offset,data);//modify by licong for drawValue()
                    }
                    ...
                    for (int j = 0; j < (valuePoints.length - 1) * mPhaseX; j += 2) {
                        ...
                        float[] vals = e.getVals();
                        String data = (String)e.getData();//add by licong for different unit
                        ...
                        if (vals == null) {
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                drawValue(mValueFormat.format(vals[k / 2]), valuePoints[j],
                                        transformed[k + 1] + offset,"");//modify by licong for drawValue()
                            }
                        }
                    }
                }
     ...
     * @param yPos
     */
    private void drawValue(String val, float xPos, float yPos,String data) {

        if (mDrawUnitInChart) {

//            mDrawCanvas.drawText(val + mUnit, xPos, yPos,
//                    mValuePaint); //remove by licong for different unit
            
            mDrawCanvas.drawText(data, xPos, yPos,
                    mValuePaint);//add by licong for different unit

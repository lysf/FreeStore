...
public abstract class Chart<T extends ChartData<? extends DataSet<? extends Entry>>> extends View
    ...
    protected void init() {
        ...
        mDebugPaint.setStrokeWidth(6f);
    }

    /**
     * add by licong
     * 重新初始化需要高亮的点的数组，将需要高亮的点标注
     */
    public void setmIndicesToHightlight(int index){
        markViewIndex = index;
        mIndicesToHightlight = new Highlight[] {
                new Highlight(index,0)};
    }
    ...
    protected void calcFormats() {
        ...
        if (!mUseCustomFormatter) {
            ...
            /**remove by licong  begin 不按照他的规则保留小数*/
...
//            mValueFormat = new DecimalFormat("###,###,###,##0" + b.toString());
            /**remove by licong  end */
            /**add by licong 只保留两位小数*/
            mValueFormat = new DecimalFormat("###,###,###,##0" + ".00");
        }
    }
     ...
     * @param pixel
     */
    public void setValueTextSize(float pixel) {
        mValuePaint.setTextSize(pixel);
    }

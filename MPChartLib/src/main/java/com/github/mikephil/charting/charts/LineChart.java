        ...
        if (mDrawYValues && mCurrentData.getYValCount() < mMaxVisibleCount * mScaleX) {
            ...
            for (int i = 0; i < mCurrentData.getDataSetCount(); i++) {
                ...
                for (int j = 0; j < positions.length * mPhaseX; j += 2) {
                    /**
                     * add by licong start
                     * 如果当前的坐标已经标注为高亮，则修改字体大小和颜色
                     * */
                    if (j == markViewIndex * 2) {
                        mValuePaint.setColor(Color.rgb(255, 0, 0));
                    ...
                    if (mDrawUnitInChart) {
                        /**
                         * add by licong start
                         * 如果当前是第一个坐标的标值时，画的时候将x偏移量再增加之前偏移量的四分之一
                         * 这样就可以贴住y轴避免挡住
                         * */
                        if (j == 0) {
                            if (j == markViewIndex * 2) {
                                mDrawCanvas.drawText(mValueFormat.format(val) + mUnit, positions[j] + positions[j] / 2,
                                        positions[j + 1]
                                                - valOffset, mValuePaint);
                            } else {
                                mDrawCanvas.drawText(mValueFormat.format(val) + mUnit, positions[j] + positions[j] / 4,
                                        positions[j + 1]
                                                - valOffset, mValuePaint);
                            }
                        } else {
                        ...
                        if (j == 0) {
                            if (j == markViewIndex * 2) {
                                mDrawCanvas.drawText(mValueFormat.format(val), positions[j] + positions[j] / 2,
                                        positions[j + 1]
                                                - valOffset, mValuePaint);
                            } else {
                                mDrawCanvas.drawText(mValueFormat.format(val), positions[j] + positions[j] / 4,
                                        positions[j + 1]
                                                - valOffset, mValuePaint);
                            }
                        } else {
